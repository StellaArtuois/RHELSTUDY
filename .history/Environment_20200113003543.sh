...
sudo yum install -y -qq system-config-kickstart -y &&

# Terminal Browser
sudo yum install -y -qq elinks -y &&

# SeLinux Troubleshoot program
sudo yum install -y -qq setroubleshoot setools -y &&

# Cockpit is a web-based server manager https://cockpit-project.org/

sudo yum install -y -qq cockpit -y &&

sudo yum install -y -qq rsync -y &&

sudo yum install -y -qq pykickstart -y &&

# Auditing with Auditd Tool on CentOS/RHEL https://www.tecmint.com/linux-system-auditing-with-auditd-tool-on-centos-rhel/

sudo yum install -y -qq auditd -y &&
